...
\hline
GLM-assisted oracle calibration & On a fixed single-seed 40-task sample, GLM emits parseable non-vacuous candidate predicates on 40/40 tasks and recalls 19/20 objective positives, Wilson 95\% CI 0.764--0.991. The GLM candidate gate alone also marks 10/20 objective negatives, CI 0.299--0.701, whereas QG-DCA admits 19/20 positives and 0/20 negatives, CI 0--0.161, after the objective patch-differential gate & Supports the generator/admission separation: a strong model can create candidate oracles, but the accounting gate decides whether they become capital \\
\hline
...
Finally, we test whether a stronger generator should be treated as a judge. In LXXXVI, GLM is given patch and context fields and asked to emit a candidate security predicate, but admission is still decided by the objective patch-differential gate. Table~\ref{tab:lxxxvi_glm_oracle_calibration} shows the result. GLM is useful as a candidate-oracle generator: it returns parseable, non-vacuous predicates on 40/40 tasks, with Wilson 95\% CI 0.912--1.000, and covers 19/20 objective positives. However, a GLM-only candidate gate also marks 10/20 objective negatives. This 0.50 false-candidate rate is prompt- and sample-dependent, and is reported only to illustrate why a candidate generator is not an admission gate. QG-DCA keeps the same 19/20 positive admits while admitting 0/20 negatives because the model output must pass the external patch-differential evidence gate before it updates $H_q(t)$. The 0/20 false admission is therefore a property of the verification predicate on this sample, not a discovered GLM capability. This is the intended role of LLMs in MDDC: they accelerate candidate construction, but they do not define truth.

\begin{table}[!t]
\caption{LXXXVI GLM-assisted oracle calibration}
\label{tab:lxxxvi_glm_oracle_calibration}
\centering
\scriptsize
\renewcommand{\arraystretch}{1.12}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{|>{\raggedright\arraybackslash}p{1.45in}|>{\raggedright\arraybackslash}p{0.85in}|>{\raggedright\arraybackslash}p{0.9in}|>{\raggedright\arraybackslash}p{1.55in}|}
\hline
\textbf{Admission rule} & \textbf{Positive recall} & \textbf{Negative false admit} & \textbf{Interpretation} \\
\hline
GLM candidate predicate gate & 19/20; CI 0.764--0.991 & 10/20; CI 0.299--0.701 & Strong candidate generation, unsafe as admission truth \\
\hline
QG-DCA objective admission & 19/20; CI 0.764--0.991 & 0/20; CI 0--0.161 & Candidate retained only when patch-differential evidence qualifies it \\
\hline
\end{tabular}
}
\end{table}
